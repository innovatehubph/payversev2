...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh balance every 15 seconds for real-time sync
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-sm ${
                        tx.type === 'received' || tx.type === 'deposit' || tx.type === 'crypto_topup' 
                          ? 'bg-green-100 text-green-700' 
                          : 'bg-gray-100 text-gray-600'
                      }`}>
                        {tx.walletType === 'phpt' ? (
                          <Coins className="h-5 w-5" />
                        ) : tx.counterparty?.fullName?.substring(0, 2).toUpperCase() || (
                          tx.type === 'deposit' || tx.type === 'crypto_topup' ? <Plus className="h-5 w-5" /> : "?"
                        )}
                      </div>
                      <div>
                        <p className="font-medium text-foreground">
                          {tx.counterparty?.fullName || tx.note?.substring(0, 30) || tx.category || "Transaction"}
                        </p>
